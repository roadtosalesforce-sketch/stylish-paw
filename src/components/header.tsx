...
export function Header({announcement, shopName, locale, dict}: {announcement?: string; shopName?: string; locale: Locale; dict: Dictionary}) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-stone-200/80 bg-[#fffdf9]/95 backdrop-blur-md">
      ...
      {mobileOpen && <div className="border-t border-stone-200 bg-white px-5 py-6 md:hidden"><div className="grid gap-4">{mobileLinks.map(([label,value]) => <Link onClick={() => setMobileOpen(false)} key={value} href={`/shop?category=${value}`} className="font-semibold text-charcoal">{label}</Link>)}<Link onClick={() => setMobileOpen(false)} href="/pages/size-guide" className="border-t border-stone-200 pt-4 font-semibold text-coral">{dict.header.sizeFit}</Link></div></div>}
    </header>
  );
}
